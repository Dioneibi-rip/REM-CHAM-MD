...
import "./config.js";
import { rembots } from './plugins/serbot-code.js';
...
    console.log(chalk.bold.cyan(`La carpeta: rembots se creó correctamente.`));
  } else {
    console.log(chalk.bold.cyan(`La carpeta: rembots ya está creada.`));
  }
...
      // 🔒 Solo continuar si es una carpeta
      if (!statSync(botPath).isDirectory()) continue;
...
if (opcion === "2" || methodCode) {
  if (!conn.authState.creds.registered) {
    ...
    if (!!phoneNumber) {
      ...
      if (!Object.keys(MCC_CODES).some((v) => addNumber.startsWith(v))) {
        console.log(
          chalk.bgBlack(
      ...
      while (true) {
        ...
        if (
          addNumber.match(/^\d+$/) &&
          Object.keys(MCC_CODES).some((v) => addNumber.startsWith(v))
        ) {
          break;
        } else {
          console.log(
