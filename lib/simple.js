...
const baileysApi = baileys.default || baileys
const {
    makeWALegacySocket,
    proto,
...
const _makeWaSocket = baileysApi.makeWASocket || baileysApi.default
...
export async function makeWASocket(connectionOptions, options = {}) {
     ...
     */
    const socketFactory = global.opts['legacy'] ? makeWALegacySocket : _makeWaSocket
    if (typeof socketFactory !== 'function') throw new TypeError('No se encontró makeWASocket compatible en Ruby-Baileys')

    let conn = await socketFactory(connectionOptions)
    if (!conn || typeof conn !== 'object') throw new TypeError('Ruby-Baileys devolvió un socket inválido')
    if (!conn.ev || typeof conn.ev.on !== 'function' || typeof conn.ev.off !== 'function') {
        throw new TypeError('Ruby-Baileys devolvió un socket sin EventEmitter válido (conn.ev)')
    }
